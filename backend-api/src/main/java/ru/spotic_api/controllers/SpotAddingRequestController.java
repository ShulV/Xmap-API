...
public class SpotAddingRequestController {
    ...
    public SpotAddingRequestController(@Value("${xmap-api.base-url}") String baseUrl,
                                       ...
                                       SecurityService securityService, MessageSource messageSource) {
    ...
    public String getCreatingPage(RedirectAttributes redirectAttributes, Model model) {
        if (securityService.isAuthenticated()) {
            ...
            redirectAttributes.addFlashAttribute("warnMessage",
                    messageSource.getMessage("warn.unauthorized.cannot-add-spot", null, Locale.getDefault()));
    ...
    public String getRequestListPage(@AuthenticationPrincipal UserDetails userDetails,
        ...
        if (SpotAddingRequestStatus.PENDING_APPROVAL.name().equals(status)) {
            securityService.validateAuthority(DBCode.Authority.ACCEPTOR.name());
